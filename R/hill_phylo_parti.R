...
#'
hill_phylo_parti = function(comm, tree, q = 0, base = exp(1),
  ...
  # comm = comm[, colSums(comm) != 0] # when only 2 sp, ade4::newick2phylog has trouble, so keep zeros

  comm_sp = intersect(colnames(comm), tree$tip.label)

  if(class(tree) != "phylo") stop("tree must be an object with phylo as class")
  if(length(setdiff(tree$tip.label, comm_sp))){
    if(show.warning) warning("Some species in the phylogeny but not in comm, \n remove them from the phylogeny...")
    tree = ape::drop.tip(tree, tree$tip.label[!tree$tip.label %in% comm_sp])
  }

  if(length(setdiff(colnames(comm), comm_sp))){
    if(show.warning) warning("Some species in the comm but not in the phylogeny, \n remove them from the comm")
    comm = comm[, comm_sp]
  }
